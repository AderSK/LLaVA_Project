...
for K in K_VALUES:
    ...
    alive_features = (max_scores_per_feat > 0).nonzero(as_tuple=True)[0].tolist()
    
    if len(alive_features) < 5:
        top_5_features = alive_features
    else:
        top_5_features = random.sample(alive_features, 5)

    print(f"Total alive features: {len(alive_features)} / {D_SAE}")
    print(f"Randomly sampled 5 ALIVE features for K={K}: {top_5_features}")
    ...
    collage_filename = os.path.join(SAE_DIR, f"collage_layer{LAYER}_topk{K}_random.png")
